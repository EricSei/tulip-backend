...
private final String STARTING_URI = "http://localhost:8080/api";
	...
	void testCreateAirline() throws Exception {
		...
		MockHttpServletRequestBuilder mockRequest = post(uri)
	            .contentType(MediaType.APPLICATION_JSON)
	            .accept(MediaType.APPLICATION_JSON)
	            .content(this.mapper.writeValueAsString(al));
	          
        mvc.perform(mockRequest)
        .andExpect( status().isCreated() ) // expect 200 status code
        .andExpect( jsonPath("$", notNullValue()))
		.andExpect( jsonPath("$").value("Airline " + al.getAirlineName() + " was created") );	       
